...
impl HubSpotService {
    ...
    pub async fn batch_find_records(
        ...
        for chunk in property_values.chunks(batch_size) {
            ...
            let url = format!("https://api.hubapi.com/crm/v3/objects/{}/search", object_type);
    ...
    pub async fn get_file_by_path(&self, file_path: &str) -> Result<Option<FileInfo>> {
        // URLエンコード
        let encoded_path = url::form_urlencoded::byte_serialize(file_path.as_bytes())
            .collect::<String>();
        
        let url = format!("https://api.hubapi.com/files/v3/files/stat/{}", encoded_path);
    ...
    pub async fn upload_file_from_base64(&self, base64_data: &str, filename: &str) -> Result<String> {
        ...
        if response.status().is_success() {
            ...
            Err(anyhow!("ファイルアップロードに失敗しました: {}", response.status()))
        }
    }
    ...
    pub async fn create_note_for_record(
        ...
        if response.status().is_success() {
            ...
            Err(anyhow!("ノート作成に失敗しました: {} - {}", status, error_text))
        }
    }
    ...
    async fn get_custom_objects(&self) -> Result<Vec<crate::commands::HubSpotObject>> {
        ...
        if response.status().is_success() {
            ...
            let custom_objects: Vec<crate::commands::HubSpotObject> = schema_response
                .results
                .into_iter()
                .filter(|obj| !matches!(obj.id.as_str(), "contacts" | "companies" | "deals" | "tickets"))
                .map(|obj| crate::commands::HubSpotObject {
                    object_type_id: obj.id.clone(),
                    name: obj.name,
                    label: format!("{}", obj.labels.plural),
                })
                .collect();
            ...
            let status = response.status();
            let error_text = response.text().await.unwrap_or_else(|_| "Unknown error".to_string());
            Err(anyhow!("カスタムオブジェクト取得エラー: {} - {}", status, error_text))
        }
    }
}