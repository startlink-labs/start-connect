...
impl CsvProcessor {
    ...
    ) -> Result<(HashMap<String, FileInfo>, HashMap<String, Vec<(String, String)>>)> {
    ...
    pub fn validate_csv_files(
        ...
        if !Path::new(content_version_path).exists() {
            return Err(anyhow!("ContentVersion.csvが見つかりません: {}", content_version_path));
        }

        if !Path::new(content_document_link_path).exists() {
            return Err(anyhow!("ContentDocumentLink.csvが見つかりません: {}", content_document_link_path));
        }
