...
pub async fn process_file_mapping(
    ...
    // 1. CSVファイルの存在確認
    if let Err(e) = CsvProcessor::validate_csv_files(
    ...
    // 3. マッピング対象レコードを抽出
    let target_records = match CsvProcessor::extract_target_records(
        &content_document_link_path,
        &object_mappings,
    ) {
        Ok(records) => records,
        ...
        if let Some(mapping) = object_mappings.get(prefix) {
                ...
                .await
            {
                Ok(found_records) => {
                    ...
                    // レコードをグループ化
                    let processable_records = CsvProcessor::group_records_by_salesforce_id(
                        records,
                        &found_records,
                    );
        ...
        if let Some(mapping) = object_mappings.get(prefix) {
            ...
            for (i, record) in records.iter().enumerate() {
                ...
                emit_progress(
                    "file_processing",
                    progress,
                    &format!("処理中 ({}/{}): {}", i + 1, records.len(), record.salesforce_id),
                );
...
pub async fn analyze_csv_files(
    ...
    // CSVファイルの存在確認
    if let Err(e) = CsvProcessor::validate_csv_files(&content_version_path, &content_document_link_path) {
        return Err(e.to_string());
    }
